...
private:
	...
	void create(const_iterator b, const_iterator e);	//create(b,e)
...
void Lis<T>::push_back(const T& t) {
	Node<T>* node = new Node<T>(t);	//t valur
...
void Lis<T>::uncreate() {
	...
	while (node != b) {//역방량으로 삭제, 소멸자 해제
		Node<T>* node_delete = node;
		node = node->prev;	//이전 node으로 이동
